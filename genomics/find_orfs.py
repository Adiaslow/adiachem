...
"""Conceptual Outline
Goal:
    Identify Open Reading Frames (ORFs) in DNA sequences provided in FASTA format.

Performance Considerations:
    Attempts to use vectorization to improve performance of finding instead of searching strings.

Import Necessary Libraries:
    numpy for numerical operations.
    tqdm for progress bars during iteration.

Define DNA Sequence Manipulation Functions:
    reverseComplement(seq): Generate the reverse complement of a DNA sequence.
    getCodingFrames(seq): Generate the six reading frames of a DNA sequence.
    encodeBase(base): Encode DNA bases into numerical representations.
    encodeCodingFrames(frames): Encode DNA sequences into numerical representations.
    encodeCodons(codons): Encode codons into numerical representations.

Define ORF Identification Functions:
    findStartsStops(encodedFrames, encodedStartCodons, encodedStopCodons): Find start and stop codons in encoded DNA frames.
    extractGeneSequence(frame, startPos, stopPos, isReverse): Extract gene sequence from a DNA frame. createGene(frameIndex, startPos, stopPos, geneSequence): Create a gene record.
    getPutativeGenes(frames, startsStops, minGeneSize, largestORF): Get putative genes from DNA frames. processFrame(frameIndex, frame, startsStopsArray, minGeneSize, largestORF): Process DNA frame to find putative genes.

Define Functions for Finding ORFs:
    findOrfsInSequence(sequence, startCodons, stopCodons): Find ORFs in a DNA sequence.
    findOrfsInGenome(sequence, startCodons, stopCodons, minGeneSize, largestORF): Find ORFs in a genome.

Define Functions for Formatting Results:
    formatGeneForPrinting(geneRecord): Format a gene record for printing.
    formatGenesForPrinting(genesWithHeaders): Format genes with headers for printing.

Main Functionality (Execution):
    Read FASTA sequences using the FastAreader.
    Find ORFs in each sequence using findOrfsInGenome.
    Format the results using formatGenesForPrinting.
...
from utilities.fastaReader import FastAreader
